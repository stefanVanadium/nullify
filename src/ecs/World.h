...
// Component type indices — must match the order in World's SoA arrays
...
public:
    ...
    // Cache-friendly iteration — returns span over all slots (check alive flag)
    ...
    // Find entity with PlayerTag (returns MAX_ENTITIES if not found)
...
private:
    ...
    std::array<Renderable,  MAX_ENTITIES> m_renderables; // no {} — sf::RectangleShape has explicit ctor
